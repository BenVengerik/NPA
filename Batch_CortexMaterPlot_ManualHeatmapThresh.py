...
# --- MANUAL SETTINGS ---
HM_THRES = 100     # Heatmap Max Color (Hz)
PROF_X_MAX = 30   # Laminar Profile X-Axis Max (Hz)

# Heatmap Thresholds Dictionary
HEATMAP_THRESHOLDS = {
    "brush_contra": HM_THRES,
    "brush_ipsi": HM_THRES,
    "brush_tail": HM_THRES,
    "pinch_contra": HM_THRES,
    "pinch_ipsi": HM_THRES,
    "pinch_tail": HM_THRES
}

# Laminar Profile X-Axis Limits Dictionary
PROFILE_X_LIMITS = {
    "brush_contra": PROF_X_MAX,
    "brush_ipsi": PROF_X_MAX,
    "brush_tail": PROF_X_MAX,
    "pinch_contra": PROF_X_MAX,
    "pinch_ipsi": PROF_X_MAX,
    "pinch_tail": PROF_X_MAX
}

# Time Window
PRE_TIME = 0.25    # Seconds before stim
POST_TIME = 0.6    # Seconds after stim
...
def process_stimulus(session, stim):
    ...
    try:
        ...
        # --- HEATMAP THRESHOLD ---
        if stim in HEATMAP_THRESHOLDS:
            vmax = HEATMAP_THRESHOLDS[stim]
            print(f"✓ Heatmap: using MANUAL max {vmax} Hz")
        else:
            vmax = np.percentile(heatmap_smooth, 99.9)
            print(f"✓ Heatmap: using AUTO max {vmax:.1f} Hz")
        ...
        # === RIGHT PANEL - PROFILE ===
        # 1. Determine X-Max First (Crucial for fixed text position)
        if stim in PROFILE_X_LIMITS:
            prof_max = PROFILE_X_LIMITS[stim]
            print(f"✓ Profile: using MANUAL x-max {prof_max} Hz")
        else:
            prof_max = np.max(depth_profile) * 1.3
            print(f"✓ Profile: using AUTO x-max {prof_max:.1f} Hz")
        
        # 2. Draw Lines and Fixed-Position Labels
        current_top = s1_upper
        for layer_name in ['L1', 'L2/3', 'L4', 'L5', 'L6']:
            layer_bot = layers[layer_name]
            if not np.isnan(layer_bot):
                ...
                # FIXED: Position label at 90% of the manual/auto X-limit
                ax_profile.text(prof_max * 0.9, layer_mid, 
                               layer_name, ha='right', va='center',
        ...
        # Save
        save_path = Path("results/figures/Cortex Master Plot/Threshold") / f"{session}_{stim}_CortexMasterPlot.png"
