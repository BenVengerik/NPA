...
# 1. CONFIGURATION
# =========================================================
SESSION = "a1"        # Change this to compare animals
INCLUDE_MUA = True      
...
def plot_trial_grid(session, stim):
    ...
    for i in range(n_trials):
        ...
        # --- DRAW LAYERS & MISSING DATA ---
        current_top = data['bounds'][1]
        layers = data['layers']
        
        missing_list = UNRECORDED_LAYERS.get(session, [])

        for name in ['L1', 'L2/3', 'L4', 'L5/6']:
            bot = layers.get(name, np.nan)
            if not np.isnan(bot):
                # Check if Unrecorded
                is_missing = name in missing_list
                
                # Draw Line
                # If missing, we don't need the white line as much, but keeps context
                ax.axhline(bot, color='white' if not is_missing else '#cccccc', linestyle='-', linewidth=0.5, alpha=0.5)
                
                if is_missing:
                    # Clean Professional Look: Solid Light Grey
                    # Alpha=1.0 makes it opaque (hides the underlying 0-data)
                    ax.axhspan(bot, current_top, facecolor='#eeeeee', alpha=1.0)
                
                # Label (First Column Only)
                if col_idx == 0:
                    # SMART COLOR: Black text on light grey background, White text on dark heatmap
                    txt_color = 'black' if is_missing else 'white'
                    
                    ax.text(-PRE_TIME + 0.02, (current_top + bot)/2, name, 
                            color=txt_color, fontsize=8, va='center', fontweight='bold')
